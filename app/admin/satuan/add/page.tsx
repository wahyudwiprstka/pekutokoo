...
const AddSatuan = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess("Berhasil Membuat Satuan");
        setError("");
        res.json().then((res) => {
          console.log(res.message);
        });
      } else {
        setError("Gagal Membuat Satuan");
        setSuccess("");
        res.json().then((res) => {
          if (!success) {
    ...
    <>
      ...
      >
        <div className="w-[80vw] md:w-[60vw] lg:w-[40vw] m-auto flex-col items-center justify-center bg-white p-4 rounded-lg shadow border border-slate-200">
          <h1 className="text-2xl font-bold mb-4">Tambah Satuan Produk</h1>
          ...
          >
            <p>Berhasil Membuat Satuan Produk!</p>
          </div>
